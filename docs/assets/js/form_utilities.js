...
function fullResetForm() {
  ...
  var customModelsContainer = document.getElementById(
...
function validateConfigForm() {
  ...
  if (document.getElementById("bench-batch_scaling").checked) {
    const batch_scaling = document.getElementById(
      "bench-batch_scaling_factors"
    ).value;
    if (batch_scaling == "") {
  ...
  if (document.getElementById("bench-uq").checked) {
    ...
    if (uq_samples == "") {
      alert(
        "Uncertainty quantification number of samples are required when active"
      );
      return false;
    }
    if (isNaN(uq_samples)) {
